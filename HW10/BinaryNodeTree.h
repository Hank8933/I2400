...
int BinaryNodeTree<ItemType>::getHighestFullLevelHelper(BinaryNode<ItemType>* subTreePtr) const
{
    if (subTreePtr->getLeftChildPtr() && subTreePtr->getRightChildPtr()) {
        int left = getHighestFullLevelHelper(subTreePtr->getLeftChildPtr()), right = getHighestFullLevelHelper(subTreePtr->getRightChildPtr());
        return left != right ? max(left, right) : right + 1;
    }
