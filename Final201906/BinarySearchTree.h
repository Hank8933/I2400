...
int BinarySearchTree<ItemType>::nonLeafCount_Iter(BinaryNode<ItemType>* subtreePtr)
{
	...
	que.push(subtreePtr);
	while (!que.empty()) {
		BinaryNode<ItemType>* now = que.front();
		que.pop();
		if (now->getLeftChildPtr() || now->getRightChildPtr())
			count++;

		if (now->getLeftChildPtr())
			que.push(now->getLeftChildPtr());

		if (now->getRightChildPtr())
			que.push(now->getRightChildPtr());
	}
